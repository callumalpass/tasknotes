...
function getEventSourceName(icsEvent: ICSEvent, plugin: TaskNotesPlugin): string {
	const subscription = plugin.icsSubscriptionService
		?.getSubscriptions()
		.find((candidate) => candidate.id === icsEvent.subscriptionId);
...
export function createICSEventCard(
	...
	const sourceName = getEventSourceName(icsEvent, plugin);
