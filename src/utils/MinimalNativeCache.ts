...
export class MinimalNativeCache extends Events {
	...
	async getFilteredTasks(
		predicate?: (file: TFile, frontmatter: any) => boolean
	): Promise<TaskInfo[]> {
		const markdownFiles = this.app.vault
			.getMarkdownFiles()
			.filter((file) => this.isValidFile(file.path));
		...
		for (let i = 0; i < markdownFiles.length; i += batchSize) {
			...
			for (const file of batch) {
				...
				if (metadata?.frontmatter && this.isTaskFile(metadata.frontmatter)) {
					...
					const taskInfo = this.extractTaskInfoFromNative(
						file.path,
	...
	async getCalendarData(year: number, month: number): Promise<any> {
		...
		for (let day = 1; day <= daysInMonth; day++) {
			...
			if (taskPaths.length > 0) {
				const tasks = await Promise.all(taskPaths.map((path) => this.getTaskInfo(path)));
	...
	getAllStatuses(): string[] {
		...
		if (this.tasksByStatus.size === 0 && this.indexesBuilt) {
			console.debug(
				"MinimalNativeCache: Indexes marked as built but tasksByStatus is empty, computing statuses synchronously"
			);
	...
	getAllTags(): string[] {
		try {
			...
			console.warn(
				"MinimalNativeCache: Failed to use native getTags(), falling back to manual scan:",
				error
			);
		}
		...
		try {
			...
			if (resolvedLinks && typeof resolvedLinks === "object") {
				...
				for (const [sourcePath, targets] of Object.entries(resolvedLinks)) {
					...
					const projectsFieldName =
						this.fieldMapper?.toUserField("projects") || "projects";
					...
					const hasWikilinks = projectsField.some(
			...
			console.warn(
				"MinimalNativeCache: Failed to use native resolvedLinks for projects, falling back to manual scan:",
				error
			);
		}
				...
				if (taskInfo && taskInfo.projects) {
					...
					if (filteredProjects.length > 0) {
						filteredProjects.forEach((project) => {
							const extractedProjects = this.extractProjectNamesFromValue(
								project,
								file.path
							);
	...
	getAllProjectsWithDetails(): Array<{
		...
		for (const file of markdownFiles) {
			...
			if (metadata?.frontmatter && this.isTaskFile(metadata.frontmatter)) {
				...
				if (filteredProjects.length > 0) {
					filteredProjects.forEach((project) => {
						const projectDetails = this.extractProjectDetailsFromValue(
							project,
							file.path
						);
	 ...
	 */
	private extractProjectDetailsFromValue(
		projectValue: string,
		sourcePath: string
	): Array<{
		name: string;
	...
	private updateOverdueIndex(path: string, taskInfo: TaskInfo): void {
		...
		if (!taskInfo.recurrence) {
			...
			if (taskInfo.due && isBeforeDateSafe(taskInfo.due, today)) {
				...
				!taskInfo.due &&
				taskInfo.scheduled &&
				isBeforeDateSafe(taskInfo.scheduled, today)
			) {
				this.overdueTasks.add(path);
			}
		}
	...
	private updateProjectReferencesIndex(
		taskPath: string,
	...
	private getStatusForTaskPath(path: string): string | undefined {
		...
		try {
			const mapped = this.fieldMapper.mapFromFrontmatter(
				metadata.frontmatter,
				path,
	...
	private async waitForFreshData(file: TFile, maxAttempts = 10): Promise<void> {
		...
		if (elapsed > 3000) {
			console.warn(
				`MinimalNativeCache: Waited ${elapsed}ms for fresh data on ${file.path} - consider investigating file system performance`
			);
		}
	}
	...
	async waitForFreshTaskData(
		...
		if (elapsed > 3000) {
			console.warn(
				`MinimalNativeCache: Waited ${elapsed}ms for fresh task data on ${file.path}`,
				expectedChanges
			);
		}
	}
