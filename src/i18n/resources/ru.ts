...
export const ru: TranslationTree = {
	...
	settings: {
		...
		features: {
			inlineTasks: {
				...
				description:
					"Настройте функции встроенных задач для беспрепятственного управления задачами в любой заметке.",
			},
			overlays: {
				taskLinkToggle: {
			...
			instantConvert: {
				toggle: {
					...
					description:
						"Включить мгновенное преобразование текста в задачи с помощью сочетаний клавиш",
				},
				folder: {
					name: "Папка для преобразования встроенных задач",
					description:
						"Папка для преобразования встроенных задач. Используйте {{currentNotePath}} для относительного пути к текущей заметке",
				},
			},
			nlp: {
				header: "Обработка естественного языка",
				description:
					"Включить интеллектуальный анализ деталей задач из ввода на естественном языке.",
				enable: {
			...
			pomodoro: {
				...
				description:
					"Встроенный таймер помодоро для управления временем и отслеживания продуктивности.",
				workDuration: {
			...
			timeblocking: {
				...
				description:
					"Настройте функциональность временных блоков для легкого планирования в ежедневных заметках. В расширенном представлении календаря удерживайте Shift + клик и перетащите для создания временных блоков.",
				enableName: "Включить блокировку времени",
				enableDesc:
					"Включить функциональность временных блоков для легкого планирования в ежедневных заметках",
				showBlocksName: "Показать временные блоки",
				showBlocksDesc: "Отображать временные блоки из ежедневных заметок по умолчанию",
				usage: "Использование: В расширенном представлении календаря удерживайте Shift + перетащите для создания временных блоков. Перетащите для перемещения существующих блоков. Измените размер краев для настройки длительности.",
			},
			performance: {
		...
		defaults: {
			...
			basicDefaults: {
				...
				useParentNoteAsProject: {
					...
					description:
						"Автоматически связывать родительскую заметку как проект при использовании мгновенного преобразования задач",
				},
				defaultTimeEstimate: {
		...
		taskProperties: {
			...
			taskPriorities: {
				...
				description:
					"Настройте уровни приоритета, доступные для ваших задач. Веса приоритета определяют порядок сортировки и визуальную иерархию в представлениях задач.",
				howTheyWork: {
				...
				validationNote:
					"Примечание: У вас должен быть минимум 1 приоритет. Более высокие веса имеют приоритет в сортировке и визуальной иерархии.",
				emptyState:
		...
		appearance: {
			...
			taskFilenames: {
				...
				customTemplate: {
					...
					description:
						"Шаблон для пользовательских имен файлов. Доступные переменные: {title}, {titleLower}, {titleUpper}, {titleSnake}, {titleKebab}, {titleCamel}, {titlePascal}, {date}, {shortDate}, {time}, {time12}, {time24}, {timestamp}, {dateTime}, {year}, {month}, {monthName}, {monthNameShort}, {day}, {dayName}, {dayNameShort}, {hour}, {hour12}, {minute}, {second}, {milliseconds}, {ms}, {ampm}, {week}, {quarter}, {unix}, {unixMs}, {timezone}, {timezoneShort}, {utcOffset}, {utcOffsetShort}, {utcZ}, {zettel}, {nano}, {priority}, {priorityShort}, {status}, {statusShort}, {dueDate}, {scheduledDate}",
			...
			defaultEventVisibility: {
				...
				description:
					"Настройте, какие типы событий видимы по умолчанию при открытии расширенного календаря. Пользователи все еще могут переключать их в представлении календаря.",
				showScheduledTasks: {
			...
			timeSettings: {
				...
				endTime: {
					...
					description:
						"Самое позднее время, показываемое в представлениях временной линии (формат ЧЧ:ММ)",
					placeholder: "22:00",
				},
				initialScrollTime: {
			...
			timeblocking: {
				description:
					"Настройте функциональность временных блоков для легкого планирования в ежедневных заметках.",
				...
				modals: {
					...
					edit: {
						...
						transformFile: {
							...
							description:
								"Путь к файлу .js или .json в вашем хранилище, который преобразует данные веб-хука",
	...
	modals: {
		...
		taskCreation: {
			...
			nlPlaceholder:
				"Купить продукты завтра в 15:00 @дом #поручения\n\nДобавить детали здесь...",
			notices: {
		...
		storageLocation: {
			...
			bullets: {
				dailyNotesRequired:
					"Основной плагин ежедневных заметок должен оставаться включенным",
