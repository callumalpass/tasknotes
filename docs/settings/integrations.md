...
When TaskNotes v5 opens a collection whose only active mdbase type is an unmodified file generated by TaskNotes v4, it automatically upgrades that metadata from mdbase v0.2 to v0.3. Task notes are not rewritten, and custom collection settings are preserved. TaskNotes keeps the previous `mdbase.yaml` and task type under `.tasknotes/migrations/`, generates a legacy-compatible v0.3 type from the retained plugin settings, and restores the v0.2 files if the upgrade cannot be completed. A durable migration record allows interrupted upgrades to recover on the next startup without overwriting concurrent edits. Collections with additional, modified, or hand-maintained types are left unchanged.
