...
- (#711, #168) Added an optional task-creation setting to open the new task note after saving, either in the current tab or a new tab. Thanks to @phortx and @23426356587 for suggesting this, and to @edward-peters for the follow-up.
...
- Refactored Calendar, Kanban, Bases task creation, Bases filter-default extraction, default Base file creation/regeneration, Bases view reload cleanup, date-change detection, NLP API creation payloads, startup settings persistence and migration, task creation/update runtime contracts, FilterService runtime contracts, filter predicate evaluation, filter task grouping, sorting, and option assembly, current-note conversion planning, task title sanitization, task property-update planning, bulk task-update planning and frontmatter mutation, task archive planning, task time-tracking and time-entry deletion planning, task recurring-instance planning, task blocking-relationship planning, task property-change side effects, task-manager identification and task-info assembly, custom-field filter coercion and grouping, agenda overdue/date selection, task-card property lookup, metadata assembly/toggle wiring, render-state/class assembly, completion-state refresh, status/priority indicator rendering, secondary badge/dependency controls, title/link rendering, context-menu integration, relationship filtering/expansion rendering, quick actions, badge controls, task-modal title input, details layout transitions, mobile focus/keyboard guards, project parsing/rendering, subtask selection/rendering state, dependency list state and candidate filtering, dependency/subtask task-selector opening, action buttons, action icons, action-icon states, action-menu callbacks, action state/menu-context assembly, action-value derivation, organization/dependency and metadata field rendering, user-field state and controls, details-editor handling, task-creation form/default handling, creation subtask assignment, task-edit form/default handling, and task-edit subtask updates, Bases selection UI, Bases diagnostics, and editor/link diagnostics around smaller tested helpers, including Calendar property-event mutation planning, Calendar config-snapshot and data-signature helpers, Calendar initial-date/navigation planning, Calendar list event-mount rendering and Bases task enrichment, shared Bases formula/property adapters, Kanban task grouping, Kanban drag-and-drop updates, Kanban drop-target reconstruction and optimistic card movement, Kanban drop side-effect planning, Kanban column/swimlane ordering and creation defaults, Task List drag-and-drop insertion geometry, Task List grouped render planning, Task List drop frontmatter and side-effect planning, settings data read/write helpers, and structured debug gating, reducing the risk of regressions in future view and task changes.

## Fixed

- Made same-column manual-order drops in TaskNotes Kanban settle immediately when the optimistic card move is safe, instead of waiting for the delayed post-drop refresh.
...
- (#1681) Reduced the cost of inline checkbox convert buttons by only scanning the visible editor range instead of the full document. Thanks to @en-ot for reporting this.
