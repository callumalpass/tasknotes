...
- Custom frontmatter keys: use the exact frontmatter key, e.g., `custom_key`
- User fields: `user:<id>` where `<id>` is the ID configured in Settings → Advanced → User Fields (e.g., `user:effort`)
